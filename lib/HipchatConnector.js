...
module.exports = function Connector() {
  ...
  this.join = function(to, password, historyStanzas) {
    console.log('XMPP JOIN---------------', arguments);
    $.ready(function() {
      var room = to.split('/')[0];
      if(!joinedRooms[room]){
  ...
  this.getRooms = function(buddy, callback) {
    $.ready(function() {
      var id = 'items-' + buddy.split('@').join('--');
      var stanza = new Stanza('iq', { type: 'get', id: id, to: self.mucHost }).
      ...
      iqCallbacks[id] = function(response) {
        if(response.attrs.type === 'error') {
          ...
          response.getChild('query').getChildren('item').map(function(el) {
  ...
  this.connect = function(params) {
    ...
    conn.on('online', function(data){
      ...
      // to the MUC service.
      self.mucHost = self.host ? "conf." + self.host : "conf.hipchat.com";
      $.start();

      // self.getVCard(data.jid.toString(), function(vCard) {
      //   // now that we have our name we can let rooms be joined
      //   self.name = vCard.fn;

      //   // this is the name used to @mention us
      //   self.mention_name = vCard.nickname;

      //   // self.getRooms(data.jid.toString(), function(rooms) {
      //   //   rooms.forEach(function(room) {
      //   //     self.join(room.jid + '/' + self.name);
      //   //   });
      //   // })
      // })
      ...
      if (stanza.is('message')) {
        ...
        if(stanza.attrs.type == 'chat') {
          ...
          if(chatstate) {
            ...
            //
            // See <Connector#setChatstate> for details on chatstates.
            //
        ...
        } else if (!stanza.attrs.type) { // invite
          ...
          events.emit('invite', inviteRoom.bare(), inviteSender.bare(), reason);
        };
...
// Connector.prototype.Element = xmpp.Element
